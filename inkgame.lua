...
DalgonaPlayerBox:AddButton("ダルゴナ即クリア", function()
    -- ダルゴナ即クリア処理
end)
...
-- Hide and Seek Tab --------------
-----------------------------------
local HideAndSeekTab = Window:AddTab("かくれんぼ", "visibility")
local HideSeekFeaturesBox = HideAndSeekTab:AddLeftGroupbox("かくれんぼ Features")
local HideSeekUtilitiesBox = HideAndSeekTab:AddRightGroupbox("かくれんぼ Utilities")
...
HideSeekUtilitiesBox:AddButton("Teleport 100 Blocks", function()
    local RootPart = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if RootPart then
        local Pos = RootPart.Position
        RootPart.CFrame = CFrame.new(Vector3.new(Pos.X, Pos.Y + 100, Pos.Z))
    end
end)
...
HideSeekUtilitiesBox:AddButton("Teleport 40 Blocks Down", function()
    ...
    if RootPart then
        local Pos = RootPart.Position
        RootPart.CFrame = CFrame.new(Vector3.new(Pos.X, Pos.Y - 40, Pos.Z))
    end
end)
...
local KillAuraBox = HideAndSeekTab:AddRightGroupbox("Kill Aura")

KillAuraBox:AddToggle("KillAuraSafe", {
    Text = "KILL AURA",
    Default = false,
    Callback = function(State)
        if not State then return end
        -- Kill Aura 処理をここに実装可能
    end
})
